...
function ImportResume({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function parse() { if (text.trim().length < 30) { setErr("Paste the résumé text first."); return; } setBusy(true); setErr(""); try { const { data, error } = await supabase.functions.invoke("ai-assist", { body: { action: "parse", text } }); if (error) throw error; if ((data as any).error) throw new Error((data as any).error); setP((data as any).parsed || {}); } catch (e: any) { setErr("AI parse failed: " + (e?.message || e)); } setBusy(false); }
  async function save() { if (!p?.first_name) { setErr("No name parsed — edit the fields or try again."); return; } setBusy(true); const { error } = await supabase.from("candidates").insert({ first_name: p.first_name, last_name: p.last_name || "-", email: p.email || null, phone: p.phone || null, current_title: p.current_title || null, current_company: p.current_company || null, experience_years: p.experience_years || null, skills: Array.isArray(p.skills) ? p.skills : null, resume_text: text, status: "new", source: "resume" }); setBusy(false); if (error) setErr(error.message); else onSaved(); }
...
function Det({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  ...
  async function smsC() { if (!c?.phone) { alert("No phone number on file for this candidate."); return; } const m = window.prompt(`Text message to ${c.first_name}:`); if (!m) return; const { data, error } = await supabase.functions.invoke("twilio", { body: { action: "sms", to: c.phone, body: m, candidate_id: c.id } }); if (error || (data as any)?.error) alert("SMS failed: " + ((data as any)?.error || error?.message)); else { alert("Text sent."); load(); } }
  async function callC() { if (!c?.phone) { alert("No phone number on file."); return; } const { data, error } = await supabase.functions.invoke("twilio", { body: { action: "call", to: c.phone, candidate_id: c.id } }); if (error || (data as any)?.error) alert("Call failed: " + ((data as any)?.error || error?.message)); else { alert("Calling — your phone rings first, then connects to the candidate."); load(); } }
  async function enrichC() { if (!c) return; const { data, error } = await supabase.functions.invoke("apollo-enrich", { body: { candidate_id: c.id } }); if (error || (data as any)?.error) alert("Apollo: " + ((data as any)?.error || error?.message)); else { const r: any = data; alert(r.matched ? ("Enriched" + (r.updated?.length ? ": " + r.updated.join(", ") : " — no new fields")) : "No Apollo match found."); load(); } }
