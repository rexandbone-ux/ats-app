...
import { errorMessage, logError, must, mustCount, mustRows, reportError, tryRun } from "@/lib/errors";
import { useAuth, can, canEdit, canManageUsers, ROLE_LABELS, Role, Section } from "@/lib/auth";

// Loosely typed Supabase row, aliased so the untyped reads have one name.
...
function Loading() { return <div className="py-20 text-center text-gray-400">Loading...</div>; }
function ErrorBox({ msg, onRetry }: { msg: string; onRetry?: () => void }) {
  return <div className="bg-red-50 border border-red-200 rounded-xl p-4 my-2 flex items-start justify-between gap-3"><div className="text-sm text-red-700">{msg}</div>{onRetry && <button onClick={onRetry} className="text-xs px-2 py-1 border border-red-300 text-red-700 rounded-lg shrink-0">Retry</button>}</div>;
}
// Best-effort: a failed audit-trail write must not abort the caller's action, but it is logged.
async function logActivity(type: string, description: string, refs: { candidate_id?: string; job_id?: string; client_id?: string; application_id?: string }, userId?: string) {
  return tryRun("log activity", supabase.from("activities").insert({ type, description, ...refs, user_id: userId || null }));
}
...
function Dash({ nav }: { nav: (p: string, d?: any) => void }) {
  ...
  const load = useCallback(async () => { setErr(""); try {
    const [ca, jo, ap, cl, st, iv, tk, calls] = await Promise.all([
      mustRows<Row>("dashboard candidates", supabase.from("candidates").select("id,status,source,created_at,updated_at").limit(10000)),
      mustRows<Row>("dashboard jobs", supabase.from("jobs").select("id,title,status,created_at,updated_at").limit(3000)),
      mustRows<Row>("dashboard applications", supabase.from("applications").select("offer_accepted_at,offer_declined_at,offer_date").limit(20000)),
      mustRows<Row>("dashboard clients", supabase.from("clients").select("id,status").limit(2000)),
      mustRows<Row>("dashboard stages", supabase.from("pipeline_stages").select("*").order("sort_order")),
      mustRows<Row>("dashboard interviews", supabase.from("interviews").select("id,scheduled_at,type,status,candidates(first_name,last_name)").gte("scheduled_at", new Date().toISOString()).order("scheduled_at").limit(8)),
      mustRows<Row>("dashboard tasks", supabase.from("tasks").select("id,title,due_date,priority,status,candidates(first_name,last_name)").neq("status", "completed").order("due_date", { nullsFirst: false }).limit(10)),
      mustRows<Row>("dashboard calls", supabase.from("activities").select("id,type,description,created_at,candidates(first_name,last_name)").in("type", ["call", "text"]).order("created_at", { ascending: false }).limit(8)),
    ]);
    ...
    setD({
      ...
      stages: st, sb, iv, tasks: tk, calls,
    });
  } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  if (err) return <ErrorBox msg={`Couldn't load the dashboard: ${err}`} onRetry={load} />;
  if (!d) return <Loading />;
...
function Cands({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const { profile } = useAuth();
  const [cs, setCs] = useState<C[]>([]); const [q, setQ] = useState(""); const [f, setF] = useState("all"); const [own, setOwn] = useState("all"); const [pg, setPg] = useState(0); const [tot, setTot] = useState(0); const [add, setAdd] = useState(false); const [sel, setSel] = useState<Set<string>>(new Set()); const [bulkTag, setBulkTag] = useState(""); const [pools, setPools] = useState<any[]>([]); const [recs, setRecs] = useState<any[]>([]); const [imp, setImp] = useState(false); const [err, setErr] = useState("");
  const recMap: Record<string, string> = {}; recs.forEach(r => { recMap[r.id] = r.first_name || r.email || "?"; });
  const load = useCallback(async () => { setErr(""); let qr = supabase.from("candidates").select("*", { count: "exact" }); if (q) qr = qr.or(`first_name.ilike.%${q}%,last_name.ilike.%${q}%,email.ilike.%${q}%,current_title.ilike.%${q}%`); if (f !== "all") qr = qr.eq("status", f); if (own === "me" && profile) qr = qr.eq("owner_id", profile.id); else if (own === "unassigned") qr = qr.is("owner_id", null); else if (own !== "all") qr = qr.eq("owner_id", own); const { data, count, error } = await qr.order("created_at", { ascending: false }).range(pg * 25, (pg + 1) * 25 - 1); if (error) { setErr(logError("load candidates", error)); return; } setCs(data || []); setTot(count || 0); }, [q, f, own, pg, profile]);
  useEffect(() => { load(); }, [load]);
  useEffect(() => { mustRows<Row>("load recruiters", supabase.from("profiles").select("id,first_name,last_name,email").in("role", ["super_admin", "admin", "recruiter", "hiring_manager"])).then(setRecs).catch(e => logError("load recruiters", e)); }, []);
  const ids = () => Array.from(sel);
  function toggle(id: string) { setSel(p => { const n = new Set(p); n.has(id) ? n.delete(id) : n.add(id); return n; }); }
  async function bulkStatus(s: string) { if (!sel.size) return; try { await must("bulk status", supabase.from("candidates").update({ status: s }).in("id", ids())); setSel(new Set()); } catch (e) { reportError("bulk status", e, "Status update"); } load(); }
  async function applyBulkTag() { const t = bulkTag.trim(); if (!t || !sel.size) return; try { for (const c of cs.filter(x => sel.has(x.id))) { const tags = Array.from(new Set([...(((c as any).tags) || []), t])); await must("bulk tag", supabase.from("candidates").update({ tags }).eq("id", c.id)); } setBulkTag(""); setSel(new Set()); } catch (e) { reportError("bulk tag", e, "Tagging"); } load(); }
  useEffect(() => { mustRows<Row>("load pools", supabase.from("talent_pools").select("id,name").order("name")).then(setPools).catch(e => logError("load pools", e)); }, []);
  async function applyBulkPool(poolId: string) { if (!poolId || !sel.size) return; const rows = ids().map(cid => ({ talent_pool_id: poolId, candidate_id: cid })); try { await must("add to pool", supabase.from("talent_pool_candidates").insert(rows)); setSel(new Set()); alert("Added to pool."); } catch (e) { reportError("add to pool", e, "Adding to pool"); } }
  return <div>
    <div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Candidates ({tot})</h1>{editable && <div className="flex gap-2"><button onClick={() => setImp(true)} className="border px-3 py-1.5 rounded-lg text-sm">✦ Import résumé</button><button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Add candidate</button></div>}</div>
    {err && <ErrorBox msg={`Couldn't load candidates: ${err}`} onRetry={load} />}
    ...
    {imp && <ImportResume onClose={() => setImp(false)} onSaved={() => { setImp(false); load(); }} />}
  </div>;
}
function ImportResume({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  const [text, setText] = useState(""); const [p, setP] = useState<any>(null); const [busy, setBusy] = useState(false); const [err, setErr] = useState("");
  async function parse() { if (text.trim().length < 30) { setErr("Paste the résumé text first."); return; } setBusy(true); setErr(""); try { const data = await invokeFunction<Row>("ai-assist", { action: "parse", text }); setP(data?.parsed || {}); } catch (e) { setErr("AI parse failed: " + logError("ai parse", e)); } setBusy(false); }
  async function save() { if (!p?.first_name) { setErr("No name parsed — edit the fields or try again."); return; } setBusy(true); const message = await tryRun("import candidate", supabase.from("candidates").insert({ first_name: p.first_name, last_name: p.last_name || "-", email: p.email || null, phone: p.phone || null, current_title: p.current_title || null, current_company: p.current_company || null, experience_years: p.experience_years || null, skills: Array.isArray(p.skills) ? p.skills : null, resume_text: text, status: "new", source: "resume" })); setBusy(false); if (message) setErr(message); else onSaved(); }
  return <Modal title="Import from résumé" onClose={onClose}>
    {!p ? <><label className="block mb-3"><span className="text-xs text-gray-500">Paste résumé text</span><textarea value={text} onChange={e => setText(e.target.value)} rows={9} placeholder="Paste the full résumé here…" className="w-full px-3 py-2 border rounded-lg text-sm mt-1" /></label>{err && <p className="text-xs text-red-500 mb-2">{err}</p>}<button disabled={busy} onClick={parse} className="w-full bg-slate-800 text-white py-2 rounded-lg text-sm">{busy ? "Parsing with AI…" : "✦ Parse with AI"}</button></>
      : <><div className="grid grid-cols-2 gap-2"><Field label="First name" value={p.first_name || ""} onChange={(e: any) => setP({ ...p, first_name: e.target.value })} /><Field label="Last name" value={p.last_name || ""} onChange={(e: any) => setP({ ...p, last_name: e.target.value })} /><Field label="Email" value={p.email || ""} onChange={(e: any) => setP({ ...p, email: e.target.value })} /><Field label="Phone" value={p.phone || ""} onChange={(e: any) => setP({ ...p, phone: e.target.value })} /><Field label="Title" value={p.current_title || ""} onChange={(e: any) => setP({ ...p, current_title: e.target.value })} /><Field label="Company" value={p.current_company || ""} onChange={(e: any) => setP({ ...p, current_company: e.target.value })} /></div>{p.skills?.length > 0 && <div className="mb-3 flex flex-wrap gap-1">{p.skills.map((s: string) => <span key={s} className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-[10px]">{s}</span>)}</div>}{err && <p className="text-xs text-red-500 mb-2">{err}</p>}<button disabled={busy} onClick={save} className="w-full bg-slate-800 text-white py-2 rounded-lg text-sm">{busy ? "Saving…" : "Save candidate"}</button></>}
  </Modal>;
}
function AddCandidate({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function save() { if (!f.first_name) { setErr("First name required"); return; } setBusy(true); const message = await tryRun("add candidate", supabase.from("candidates").insert({ ...f, status: "new", source: "manual" })); setBusy(false); if (message) setErr(message); else onSaved(); }
...
function Det({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  const { profile } = useAuth();
  const [c, setC] = useState<any>(null); const [tab, setTab] = useState("overview"); const [apps, setApps] = useState<any[]>([]); const [notes, setNotes] = useState<any[]>([]); const [note, setNote] = useState(""); const [scoring, setScoring] = useState(false); const [tag, setTag] = useState(""); const [emailOpen, setEmailOpen] = useState(false); const [acts, setActs] = useState<any[]>([]); const [recruiters, setRecruiters] = useState<any[]>([]); const [jobsList, setJobsList] = useState<any[]>([]); const [stages, setStages] = useState<any[]>([]); const [addPipe, setAddPipe] = useState(false); const [err, setErr] = useState("");
  useEffect(() => {
    mustRows<Row>("load recruiters", supabase.from("profiles").select("id,first_name,last_name,email,role").in("role", ["super_admin", "admin", "recruiter", "hiring_manager"])).then(setRecruiters).catch(e => logError("load recruiters", e));
    mustRows<Row>("load jobs", supabase.from("jobs").select("id,title").order("created_at", { ascending: false })).then(setJobsList).catch(e => logError("load jobs", e));
    mustRows<Row>("load stages", supabase.from("pipeline_stages").select("id,name,sort_order").order("sort_order")).then(setStages).catch(e => logError("load stages", e));
  }, []);
  const load = useCallback(async () => {
    setErr("");
    try {
      setC(await must("load candidate", supabase.from("candidates").select("*").eq("id", pr.id).single()));
      setApps(await mustRows<Row>("load applications", supabase.from("applications").select("id,status,created_at,job_id,offer_amount,offer_date,offer_accepted_at,offer_declined_at,jobs(title)").eq("candidate_id", pr.id)));
      setNotes(await mustRows<Row>("load notes", supabase.from("notes").select("*").eq("candidate_id", pr.id).order("created_at", { ascending: false })));
      setActs(await mustRows<Row>("load activities", supabase.from("activities").select("*").eq("candidate_id", pr.id).order("created_at", { ascending: false }).limit(100)));
    } catch (e) { setErr(errorMessage(e)); }
  }, [pr.id]);
  useEffect(() => { load(); }, [load]);
  async function setStatus(s: string) { if (!c) return; const prev = c.status; try { await must("set status", supabase.from("candidates").update({ status: s }).eq("id", c.id)); } catch (e) { reportError("set status", e, "Status update"); return; } setC({ ...c, status: s }); logActivity("stage_change", `Status: ${prev} → ${s}`, { candidate_id: c.id }, profile?.id).then(load); }
  async function addNote() { if (!note.trim()) return; try { await must("add note", supabase.from("notes").insert({ candidate_id: pr.id, content: note, user_id: profile?.id })); } catch (e) { reportError("add note", e, "Saving the note"); return; } await logActivity("note", note.slice(0, 140), { candidate_id: pr.id }, profile?.id); setNote(""); load(); }
  async function addTag() { const t = tag.trim(); if (!t || !c) return; const tags = Array.from(new Set([...(c.tags || []), t])); try { await must("add tag", supabase.from("candidates").update({ tags }).eq("id", c.id)); } catch (e) { reportError("add tag", e, "Adding the tag"); return; } setC({ ...c, tags }); setTag(""); }
  async function removeTag(t: string) { if (!c) return; const tags = (c.tags || []).filter((x: string) => x !== t); try { await must("remove tag", supabase.from("candidates").update({ tags }).eq("id", c.id)); } catch (e) { reportError("remove tag", e, "Removing the tag"); return; } setC({ ...c, tags }); }
  async function setOwner(uid: string) { if (!c) return; try { await must("set owner", supabase.from("candidates").update({ owner_id: uid || null }).eq("id", c.id)); } catch (e) { reportError("set owner", e, "Assigning the owner"); return; } setC({ ...c, owner_id: uid || null }); const rn = recruiters.find(r => r.id === uid); logActivity("assigned", `Assigned to ${rn ? (rn.first_name || rn.email) : "—"}`, { candidate_id: c.id }, profile?.id).then(load); }
  async function addToPipeline(jobId: string) { if (!jobId || !c) return; const stageId = stages[0]?.id || null; try { await must("add to pipeline", supabase.from("applications").insert({ candidate_id: c.id, job_id: jobId, stage_id: stageId, status: "active", source: "manual" })); await logActivity("application_received", "Added to pipeline", { candidate_id: c.id, job_id: jobId }, profile?.id); } catch (e) { reportError("add to pipeline", e, "Adding to the pipeline"); } setAddPipe(false); load(); }
  async function smsC() { if (!c?.phone) { alert("No phone number on file for this candidate."); return; } const m = window.prompt(`Text message to ${c.first_name}:`); if (!m) return; try { await invokeFunction("twilio", { action: "sms", to: c.phone, body: m, candidate_id: c.id }); } catch (e) { reportError("twilio sms", e, "SMS"); return; } alert("Text sent."); load(); }
  async function callC() { if (!c?.phone) { alert("No phone number on file."); return; } try { await invokeFunction("twilio", { action: "call", to: c.phone, candidate_id: c.id }); } catch (e) { reportError("twilio call", e, "Call"); return; } alert("Calling — your phone rings first, then connects to the candidate."); load(); }
  async function enrichC() { if (!c) return; let r: any; try { r = await invokeFunction<Row>("apollo-enrich", { candidate_id: c.id }); } catch (e) { reportError("apollo enrich", e, "Apollo enrichment"); return; } alert(r?.matched ? ("Enriched" + (r.updated?.length ? ": " + r.updated.join(", ") : " — no new fields")) : "No Apollo match found."); load(); }
  async function recordOffer(appId: string) { const amt = window.prompt("Offer amount (e.g. 5 or 1200):"); if (amt === null) return; try { await must("record offer", supabase.from("applications").update({ offer_amount: parseFloat(amt.replace(/[^0-9.]/g, "")) || null, offer_date: new Date().toISOString() }).eq("id", appId)); if (c) await must("record offer status", supabase.from("candidates").update({ status: "offered" }).eq("id", c.id)); } catch (e) { reportError("record offer", e, "Recording the offer"); load(); return; } await logActivity("offer", `Offer extended${amt ? " ($" + amt + ")" : ""}`, { candidate_id: pr.id }, profile?.id); load(); }
  async function offerDecision(appId: string, accepted: boolean) { try { await must("offer decision", supabase.from("applications").update(accepted ? { offer_accepted_at: new Date().toISOString() } : { offer_declined_at: new Date().toISOString() }).eq("id", appId)); if (c) await must("offer decision status", supabase.from("candidates").update({ status: accepted ? "placed" : "rejected" }).eq("id", c.id)); } catch (e) { reportError("offer decision", e, "Recording the decision"); load(); return; } await logActivity("offer", accepted ? "Offer accepted" : "Offer declined", { candidate_id: pr.id }, profile?.id); load(); }
  async function runAI() {
    setScoring(true);
    try { await invokeFunction("candidate-ai-analysis", { candidate_id: pr.id }); await load(); } catch (e) { reportError("ai analysis", e, "AI analysis"); }
    setScoring(false);
  }
  if (err && !c) return <div><button onClick={() => nav("cands")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button><ErrorBox msg={`Couldn't load this candidate: ${err}`} onRetry={load} /></div>;
  if (!c) return <Loading />;
  ...
  return <div>
    ...
    {err && <ErrorBox msg={`Some of this candidate's data couldn't be loaded: ${err}`} onRetry={load} />}
...
function EmailModal({ cand, onClose }: { cand: any; onClose: () => void }) {
  ...
  useEffect(() => { mustRows<Row>("load templates", supabase.from("email_templates").select("*")).then(setTmpls).catch(e => logError("load templates", e)); }, []);
  async function draftAI() { setDrafting(true); setMsg(""); try { const data = await invokeFunction<Row>("ai-assist", { action: "outreach", candidate_id: cand.id, role: cand.current_title }); setSubject(data?.subject || subject); setBodyHtml(data?.body || bodyHtml); } catch (e) { setMsg("AI draft failed: " + logError("ai draft", e)); } setDrafting(false); }
  function fill(t: any) {
  ...
  async function send() {
    ...
    setBusy(true); setMsg("");
    let status = "sent"; let sendError = "";
    try { await invokeFunction("resend-email", { to: cand.email, subject, html: bodyHtml }); } catch (e) { status = "failed"; sendError = logError("send email", e); }
    // The email row is the record of the attempt, so it is written for failures too.
    const saveError = await tryRun("save email", supabase.from("emails").insert({ candidate_id: cand.id, user_id: profile?.id, to_email: cand.email, subject, body_html: bodyHtml, direction: "outbound", status, sent_at: status === "sent" ? new Date().toISOString() : null }));
    await logActivity("email", `Email: ${subject}`, { candidate_id: cand.id }, profile?.id);
    setBusy(false);
    if (status === "sent" && !saveError) { onClose(); return; }
    if (status === "sent") setMsg(`Email sent, but it couldn't be saved to this candidate's history: ${saveError}`);
    else setMsg(`Delivery failed (${sendError}). ${saveError ? "It also couldn't be saved to this candidate's history." : "It's saved to this candidate's email history — connect a verified sender (Resend domain or Gmail) to send for real."}`);
  }
...
function Jobs({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [jobs, setJobs] = useState<any[]>([]); const [clients, setClients] = useState<any[]>([]); const [apps, setApps] = useState<any[]>([]); const [stages, setStages] = useState<any[]>([]); const [recs, setRecs] = useState<any[]>([]); const [add, setAdd] = useState(false); const [collapsed, setCollapsed] = useState<Record<string, boolean>>({}); const [err, setErr] = useState("");
  const load = useCallback(async () => {
    setErr("");
    try {
      const [j, c, a, st, r] = await Promise.all([
        mustRows<Row>("load jobs", supabase.from("jobs").select("*").order("created_at", { ascending: false })),
        mustRows<Row>("load clients", supabase.from("clients").select("id,company_name").order("company_name")),
        mustRows<Row>("load applications", supabase.from("applications").select("job_id,stage_id,status").limit(5000)),
        mustRows<Row>("load stages", supabase.from("pipeline_stages").select("id,name,sort_order,color").order("sort_order")),
        mustRows<Row>("load recruiters", supabase.from("profiles").select("id,first_name,email").in("role", ["super_admin", "admin", "recruiter", "hiring_manager"])),
      ]);
  ...
  // Optimistic updates are reverted when the write fails so the board never shows an unsaved value.
  async function setJobStatus(id: string, s: string) { const prev = jobs.find(j => j.id === id)?.status; setJobs(js => js.map(j => j.id === id ? { ...j, status: s } : j)); try { await must("set job status", supabase.from("jobs").update({ status: s }).eq("id", id)); } catch (e) { setJobs(js => js.map(j => j.id === id ? { ...j, status: prev } : j)); reportError("set job status", e, "Status update"); } }
  async function setJobRec(id: string, r: string) { const prev = jobs.find(j => j.id === id)?.recruiter_id ?? null; setJobs(js => js.map(j => j.id === id ? { ...j, recruiter_id: r || null } : j)); try { await must("set job recruiter", supabase.from("jobs").update({ recruiter_id: r || null }).eq("id", id)); } catch (e) { setJobs(js => js.map(j => j.id === id ? { ...j, recruiter_id: prev } : j)); reportError("set job recruiter", e, "Assigning the recruiter"); } }
  ...
  return <div>
    ...
    {err && <ErrorBox msg={`Couldn't load the positions board: ${err}`} onRetry={load} />}
...
function AddJob({ clients, onClose, onSaved }: { clients: Record<string, string>; onClose: () => void; onSaved: () => void }) {
  ...
  async function genJD() { if (!f.title) { setErr("Add a title first"); return; } setGen(true); try { const data = await invokeFunction<Row>("ai-assist", { action: "jd", title: f.title }); setF((x: any) => ({ ...x, description: data?.text })); } catch (e) { setErr("AI: " + logError("generate jd", e)); } setGen(false); }
  async function save() { if (!f.title) { setErr("Title required"); return; } setBusy(true); const payload: any = { title: f.title, location: f.location, description: f.description, status: "open" }; if (f.client_id) payload.client_id = f.client_id; const message = await tryRun("add job", supabase.from("jobs").insert(payload)); setBusy(false); if (message) setErr(message); else onSaved(); }
...
function JobDetail({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  const [j, setJ] = useState<any>(null); const [apps, setApps] = useState<any[]>([]); const [qs, setQs] = useState<string[]>([]); const [qIn, setQIn] = useState(""); const [err, setErr] = useState("");
  const load = useCallback(async () => {
    setErr("");
    try {
      const data: any = await must("load job", supabase.from("jobs").select("*,clients(company_name)").eq("id", pr.id).single());
      setJ(data); setQs(Array.isArray(data?.screening_questions) ? data.screening_questions.map((x: any) => typeof x === "string" ? x : (x.question || "")) : []);
      setApps(await mustRows<Row>("load applicants", supabase.from("applications").select("id,status,created_at,candidate_id,candidates(first_name,last_name,current_title,status)").eq("job_id", pr.id)));
    } catch (e) { setErr(errorMessage(e)); }
  }, [pr.id]);
  useEffect(() => { load(); }, [load]);
  async function togglePublish() { const np = !j.is_public; try { await must("publish job", supabase.from("jobs").update({ is_public: np, published_at: np ? new Date().toISOString() : null }).eq("id", j.id)); } catch (e) { reportError("publish job", e, np ? "Publishing" : "Unpublishing"); return; } setJ({ ...j, is_public: np }); }
  async function saveQs(next: string[]) { const prev = qs; setQs(next); try { await must("save screening questions", supabase.from("jobs").update({ screening_questions: next }).eq("id", j.id)); } catch (e) { setQs(prev); reportError("save screening questions", e, "Saving the questions"); } }
  if (err && !j) return <div><button onClick={() => nav("jobs")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button><ErrorBox msg={`Couldn't load this position: ${err}`} onRetry={load} /></div>;
  if (!j) return <Loading />;
  return <div><button onClick={() => nav("jobs")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button>
    {err && <ErrorBox msg={`Some of this position's data couldn't be loaded: ${err}`} onRetry={load} />}
...
function Clients({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [cs, setCs] = useState<any[]>([]); const [add, setAdd] = useState(false); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try { setCs(await mustRows<Row>("load clients", supabase.from("clients").select("*").order("company_name"))); } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Clients ({cs.length})</h1>{editable && <button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Add client</button>}</div>{err && <ErrorBox msg={`Couldn't load clients: ${err}`} onRetry={load} />}<div className="space-y-2">{cs.map(c => <div key={c.id} onClick={() => nav("client", { id: c.id })} className="bg-white rounded-xl border p-4 flex justify-between items-center cursor-pointer hover:bg-gray-50"><div className="flex items-center gap-3"><Av n={c.company_name} /><div><div className="font-medium capitalize">{c.company_name}</div><div className="text-xs text-gray-400">{c.industry || ""}</div></div></div><B s={c.status} /></div>)}</div>{add && <AddClient onClose={() => setAdd(false)} onSaved={() => { setAdd(false); load(); }} />}</div>;
}
function AddClient({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function save() { if (!f.company_name) { setErr("Company name required"); return; } setBusy(true); const message = await tryRun("add client", supabase.from("clients").insert({ ...f, status: "active" })); setBusy(false); if (message) setErr(message); else onSaved(); }
...
function ClientDetail({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  const [cl, setCl] = useState<any>(null); const [jobs, setJobs] = useState<any[]>([]); const [contacts, setContacts] = useState<any[]>([]); const [addC, setAddC] = useState(false); const [err, setErr] = useState("");
  const load = useCallback(async () => {
    setErr("");
    try {
      setCl(await must("load client", supabase.from("clients").select("*").eq("id", pr.id).single()));
      setJobs(await mustRows<Row>("load client jobs", supabase.from("jobs").select("id,title,status,location").eq("client_id", pr.id)));
      setContacts(await mustRows<Row>("load client contacts", supabase.from("client_contacts").select("*").eq("client_id", pr.id)));
    } catch (e) { setErr(errorMessage(e)); }
  }, [pr.id]);
  useEffect(() => { load(); }, [load]);
  if (err && !cl) return <div><button onClick={() => nav("clients")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button><ErrorBox msg={`Couldn't load this client: ${err}`} onRetry={load} /></div>;
  if (!cl) return <Loading />;
  return <div><button onClick={() => nav("clients")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button>
    {err && <ErrorBox msg={`Some of this client's data couldn't be loaded: ${err}`} onRetry={load} />}
...
function AddContact({ clientId, onClose, onSaved }: { clientId: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() { if (!f.first_name) { setErr("First name required"); return; } setBusy(true); const message = await tryRun("add contact", supabase.from("client_contacts").insert({ ...f, client_id: clientId })); setBusy(false); if (message) setErr(message); else onSaved(); }
...
function Pipeline({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [stages, setStages] = useState<any[]>([]); const [apps, setApps] = useState<any[]>([]); const [drag, setDrag] = useState<string | null>(null); const [loading, setLoading] = useState(true); const [recMap, setRecMap] = useState<Record<string, string>>({}); const [err, setErr] = useState("");
  const load = useCallback(async () => {
    setErr("");
    try {
      const [st, ap, prof] = await Promise.all([
        mustRows<Row>("load stages", supabase.from("pipeline_stages").select("*").order("sort_order")),
        mustRows<Row>("load pipeline", supabase.from("applications").select("id,stage_id,status,candidate_id,job_id,candidates(first_name,last_name,current_title,owner_id),jobs(title)").limit(500)),
        mustRows<Row>("load recruiters", supabase.from("profiles").select("id,first_name,last_name,email")),
      ]);
      const m: Record<string, string> = {}; prof.forEach((p: any) => { m[p.id] = p.first_name || p.email || "?"; });
      setRecMap(m); setStages(st); setApps(ap);
    } catch (e) { setErr(errorMessage(e)); }
    setLoading(false);
  }, []);
  useEffect(() => { load(); }, [load]);
  const firstStage = stages[0]?.id;
  async function drop(stageId: string) { if (!drag) return; const id = drag; setDrag(null); const prev = apps.find(x => x.id === id)?.stage_id ?? null; setApps(a => a.map(x => x.id === id ? { ...x, stage_id: stageId } : x)); try { await must("move applicant", supabase.from("applications").update({ stage_id: stageId }).eq("id", id)); } catch (e) { setApps(a => a.map(x => x.id === id ? { ...x, stage_id: prev } : x)); reportError("move applicant", e, "Moving the applicant"); } }
  if (loading) return <Loading />;
  if (err) return <div><h1 className="text-xl font-semibold mb-4">Pipeline</h1><ErrorBox msg={`Couldn't load the pipeline: ${err}`} onRetry={load} /></div>;
...
function Search({ nav }: { nav: (p: string, d?: any) => void }) {
  const [q, setQ] = useState(""); const [rs, setRs] = useState<C[]>([]); const [busy, setBusy] = useState(false); const [done, setDone] = useState(false); const [terms, setTerms] = useState<string[]>([]); const [err, setErr] = useState("");
  async function go() {
    if (!q.trim()) return; setBusy(true); setDone(false); setTerms([]); setErr("");
    let t: string[] = [];
    // A failed AI parse only degrades ranking, so it falls back to keyword terms.
    try { const data = await invokeFunction<Row>("ai-assist", { action: "search_parse", query: q }); const f = data?.filters; if (f) t = [...(f.titles || []), ...(f.skills || []), ...(f.keywords || [])]; } catch (e) { logError("ai search parse", e); }
    ...
    try { setRs(await mustRows<C>("search candidates", supabase.from("candidates").select("*").or(or).limit(40))); setDone(true); } catch (e) { setRs([]); setErr(errorMessage(e)); }
    setBusy(false);
  }
  return <div><h1 className="text-xl font-semibold mb-1">AI Search</h1><p className="text-xs text-gray-400 mb-4">Search by title, name, or skill &mdash; e.g. &ldquo;medical intake coordinator&rdquo;.</p><div className="flex gap-2 mb-4"><input value={q} onChange={e => setQ(e.target.value)} onKeyDown={e => e.key === "Enter" && go()} placeholder="Search candidates..." className="flex-1 px-3 py-2 border rounded-lg text-sm" /><button onClick={go} disabled={busy} className="bg-slate-800 text-white px-4 py-2 rounded-lg text-sm disabled:opacity-50">{busy ? "..." : "Search"}</button></div>{err && <ErrorBox msg={`Search failed: ${err}`} onRetry={go} />}{terms.length > 0 && <div className="text-[11px] text-gray-400 mb-3">AI interpreted as: {terms.map(t => <span key={t} className="inline-block px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full mr-1">{t}</span>)}</div>}
...
function Interviews({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [list, setList] = useState<any[]>([]); const [add, setAdd] = useState(false); const [fb, setFb] = useState<any>(null); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try { setList(await mustRows<Row>("load interviews", supabase.from("interviews").select("*,candidates(first_name,last_name,current_title)").order("scheduled_at", { ascending: true }))); } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  const now = new Date();
  ...
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Interviews</h1>{editable && <button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Schedule</button>}</div>
    {err && <ErrorBox msg={`Couldn't load interviews: ${err}`} onRetry={load} />}
...
function ScheduleInterview({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => { if (q.length < 2) { setOpts([]); return; } const t = setTimeout(() => { mustRows<Row>("candidate lookup", supabase.from("candidates").select("id,first_name,last_name").or(`first_name.ilike.%${q}%,last_name.ilike.%${q}%`).limit(6)).then(setOpts).catch(e => { setOpts([]); setErr(logError("candidate lookup", e)); }); }, 250); return () => clearTimeout(t); }, [q]);
  async function save() {
    if (!f.candidate_id || !f.scheduled_at) { setErr("Pick a candidate and a date/time"); return; }
    setBusy(true); setErr(""); let location = f.location; let meet: string | null = null;
    if (zoom) {
      try { const data = await invokeFunction<Row>("zoom-meeting", { topic: `Interview – ${f.candidate_name}`, start_time: new Date(f.scheduled_at).toISOString(), duration: Number(f.duration) || 30 }); meet = data?.join_url; location = meet || location; } catch (e) { setBusy(false); setErr("Zoom: " + logError("zoom meeting", e)); return; }
    }
    const message = await tryRun("schedule interview", supabase.from("interviews").insert({ candidate_id: f.candidate_id, type: f.type, scheduled_at: new Date(f.scheduled_at).toISOString(), duration: Number(f.duration) || 30, location: location || null, google_meet_link: meet, status: "scheduled" }));
    setBusy(false); if (message) { setErr(message); return; }
...
function InterviewFeedback({ iv, onClose, onSaved }: { iv: any; onClose: () => void; onSaved: () => void }) {
  const [rec, setRec] = useState(iv.recommendation || ""); const [notes, setNotes] = useState(iv.feedback_notes || ""); const [score, setScore] = useState(iv.scorecard_data?.rating || 0); const [busy, setBusy] = useState(false); const [err, setErr] = useState("");
  async function save() { setBusy(true); const message = await tryRun("save feedback", supabase.from("interviews").update({ recommendation: rec || null, feedback_notes: notes || null, scorecard_data: { rating: score }, status: "completed", feedback_submitted_at: new Date().toISOString() }).eq("id", iv.id)); setBusy(false); if (message) { setErr(message); return; } onSaved(); }
...
function Tasks() {
  const { profile } = useAuth();
  const [list, setList] = useState<any[]>([]); const [add, setAdd] = useState(false); const [filter, setFilter] = useState("open"); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try { setList(await mustRows<Row>("load tasks", supabase.from("tasks").select("*,candidates(first_name,last_name)").order("due_date", { ascending: true }))); } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  async function toggle(t: any) { try { await must("toggle task", supabase.from("tasks").update({ status: t.status === "completed" ? "pending" : "completed", completed_at: t.status === "completed" ? null : new Date().toISOString() }).eq("id", t.id)); } catch (e) { reportError("toggle task", e, "Updating the task"); } load(); }
  ...
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Tasks</h1><div className="flex gap-2"><select value={filter} onChange={e => setFilter(e.target.value)} className="px-2 py-1.5 border rounded-lg text-sm"><option value="open">Open</option><option value="completed">Completed</option><option value="all">All</option></select><button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Add task</button></div></div>
    {err && <ErrorBox msg={`Couldn't load tasks: ${err}`} onRetry={load} />}
...
function AddTask({ me, onClose, onSaved }: { me?: string; onClose: () => void; onSaved: () => void }) {
  ...
  async function save() { if (!f.title) { setErr("Title required"); return; } setBusy(true); const message = await tryRun("add task", supabase.from("tasks").insert({ title: f.title, due_date: f.due_date ? new Date(f.due_date).toISOString() : null, priority: f.priority, status: "pending", created_by_id: me || null, assigned_to_id: me || null })); setBusy(false); if (message) setErr(message); else onSaved(); }
...
function Reports() {
  const [d, setD] = useState<any>(null); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try {
    const [ca, ap, iv, openJobs, placedCount] = await Promise.all([
      mustRows<Row>("report candidates", supabase.from("candidates").select("status,source,roe_rating").limit(10000)),
      mustRows<Row>("report applications", supabase.from("applications").select("status,created_at").limit(20000)),
      mustRows<Row>("report interviews", supabase.from("interviews").select("status,recommendation")),
      mustCount("report open jobs", supabase.from("jobs").select("id", { count: "exact", head: true }).eq("status", "open")),
      mustCount("report placed", supabase.from("candidates").select("id", { count: "exact", head: true }).eq("status", "placed")),
    ]);
    ...
    setD({ totalCand: ca.length, apps: ap.length, jobsOpen: openJobs, placed: placedCount, bySource, byStatus, ivTotal: iv.length, ivDone, funnel: ["new", "contacted", "screening", "interviewing", "offered", "placed"].map(s => ({ s, n: ca.filter((c: any) => c.status === s).length })) });
  } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  if (err) return <div><h1 className="text-xl font-semibold mb-4">Reports</h1><ErrorBox msg={`Couldn't load reports: ${err}`} onRetry={load} /></div>;
  if (!d) return <Loading />;
...
function Placements({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [list, setList] = useState<any[]>([]); const [add, setAdd] = useState(false); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try { setList(await mustRows<Row>("load placements", supabase.from("placements").select("*,candidates(first_name,last_name),clients(company_name),jobs(title)").order("created_at", { ascending: false }))); } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Placements ({list.length})</h1>{editable && <button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Record placement</button>}</div>
    {err && <ErrorBox msg={`Couldn't load placements: ${err}`} onRetry={load} />}
...
function RecordPlacement({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  useEffect(() => {
    mustRows<Row>("load clients", supabase.from("clients").select("id,company_name").order("company_name")).then(setClients).catch(e => setErr(logError("load clients", e)));
    mustRows<Row>("load jobs", supabase.from("jobs").select("id,title").order("created_at", { ascending: false })).then(setJobs).catch(e => setErr(logError("load jobs", e)));
  }, []);
  useEffect(() => { if (q.length < 2) { setOpts([]); return; } const t = setTimeout(() => { mustRows<Row>("candidate lookup", supabase.from("candidates").select("id,first_name,last_name").or(`first_name.ilike.%${q}%,last_name.ilike.%${q}%`).limit(6)).then(setOpts).catch(e => { setOpts([]); setErr(logError("candidate lookup", e)); }); }, 250); return () => clearTimeout(t); }, [q]);
  async function save() {
    if (!f.candidate_id || !f.client_id) { setErr("Pick a candidate and a client"); return; }
    setBusy(true); setErr("");
    const message = await tryRun("record placement", supabase.from("placements").insert({ candidate_id: f.candidate_id, client_id: f.client_id, job_id: f.job_id || null, status: "active", start_date: f.start_date || null, candidate_pay_rate: f.candidate_pay_rate ? Number(f.candidate_pay_rate) : null, client_bill_rate: f.client_bill_rate ? Number(f.client_bill_rate) : null, pay_period: f.pay_period }));
    if (message) { setBusy(false); setErr(message); return; }
    // The placement row is saved; a failed status sync is surfaced without losing it.
    const statusError = await tryRun("mark candidate placed", supabase.from("candidates").update({ status: "placed" }).eq("id", f.candidate_id));
    ...
    setBusy(false);
    if (statusError) { setErr(`Placement saved, but the candidate's status couldn't be set to placed: ${statusError}`); return; }
    onSaved();
  }
...
function Pools({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [pools, setPools] = useState<any[]>([]); const [counts, setCounts] = useState<Record<string, number>>({}); const [add, setAdd] = useState(false); const [name, setName] = useState(""); const [err, setErr] = useState(""); const [createErr, setCreateErr] = useState("");
  const load = useCallback(async () => {
    setErr("");
    try {
      setPools(await mustRows<Row>("load pools", supabase.from("talent_pools").select("*").order("name")));
      const tpc = await mustRows<Row>("load pool members", supabase.from("talent_pool_candidates").select("talent_pool_id").limit(20000));
      const c: Record<string, number> = {}; tpc.forEach((r: any) => { c[r.talent_pool_id] = (c[r.talent_pool_id] || 0) + 1; }); setCounts(c);
    } catch (e) { setErr(errorMessage(e)); }
  }, []);
  useEffect(() => { load(); }, [load]);
  async function create() { if (!name.trim()) return; const message = await tryRun("create pool", supabase.from("talent_pools").insert({ name: name.trim() })); if (message) { setCreateErr(message); return; } setName(""); setCreateErr(""); setAdd(false); load(); }
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Talent pools ({pools.length})</h1>{editable && <button onClick={() => setAdd(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ New pool</button>}</div>
    {err && <ErrorBox msg={`Couldn't load talent pools: ${err}`} onRetry={load} />}
    ...
    {add && <Modal title="New talent pool" onClose={() => setAdd(false)}><Field label="Pool name *" value={name} onChange={(e: any) => setName(e.target.value)} />{createErr && <p className="text-xs text-red-500 mb-2">{createErr}</p>}<button onClick={create} className="w-full bg-slate-800 text-white py-2 rounded-lg text-sm">Create pool</button></Modal>}
  </div>;
}
function PoolDetail({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  ...
  const load = useCallback(async () => { setErr(""); try { setMembers(await mustRows<Row>("load pool members", supabase.from("talent_pool_candidates").select("id,candidate_id,candidates(first_name,last_name,current_title,status)").eq("talent_pool_id", pr.id))); } catch (e) { setErr(errorMessage(e)); } }, [pr.id]);
  useEffect(() => { load(); }, [load]);
  async function remove(rowId: string) { try { await must("remove from pool", supabase.from("talent_pool_candidates").delete().eq("id", rowId)); } catch (e) { reportError("remove from pool", e, "Removing the candidate"); } load(); }
  return <div><button onClick={() => nav("pools")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button>
    ...
    {err && <ErrorBox msg={`Couldn't load this pool: ${err}`} onRetry={load} />}
...
function Settings() {
  const { profile } = useAuth();
  const [users, setUsers] = useState<any[]>([]); const [invite, setInvite] = useState(false); const [clients, setClients] = useState<any[]>([]); const [err, setErr] = useState("");
  const load = useCallback(async () => { setErr(""); try { setUsers(await mustRows<Row>("load users", supabase.from("profiles").select("id,email,first_name,last_name,role,is_active,client_id").order("created_at", { ascending: true }))); } catch (e) { setErr(errorMessage(e)); } }, []);
  useEffect(() => { load(); mustRows<Row>("load clients", supabase.from("clients").select("id,company_name").order("company_name")).then(setClients).catch(e => logError("load clients", e)); }, [load]);
  const ROLES: Role[] = ["super_admin", "admin", "recruiter", "hiring_manager", "client_user", "candidate_user"];
  // Role and access writes are re-read from the server, so a failure must be loud rather than leaving a stale UI.
  async function setRole(id: string, role: string) { try { await must("set role", supabase.from("profiles").update({ role }).eq("id", id)); } catch (e) { reportError("set role", e, "Changing the role"); } load(); }
  async function setClient(id: string, client_id: string) { try { await must("set user client", supabase.from("profiles").update({ client_id: client_id || null }).eq("id", id)); } catch (e) { reportError("set user client", e, "Assigning the client"); } load(); }
  async function toggle(id: string, active: boolean) { try { await must("toggle user", supabase.from("profiles").update({ is_active: !active }).eq("id", id)); } catch (e) { reportError("toggle user", e, active ? "Disabling the user" : "Enabling the user"); } load(); }
  return <div><div className="flex justify-between items-center mb-4"><h1 className="text-xl font-semibold">Team &amp; access</h1><button onClick={() => setInvite(true)} className="bg-slate-800 text-white px-3 py-1.5 rounded-lg text-sm">+ Add employee</button></div>
    {err && <ErrorBox msg={`Couldn't load the team list: ${err}`} onRetry={load} />}
...
function InviteUser({ onClose, onSaved }: { onClose: () => void; onSaved: () => void }) {
  ...
  async function save() {
    ...
    setBusy(true); setMsg("");
    try { await invokeFunction("admin-create-user", f); onSaved(); }
    catch (e) { setMsg("Could not create user: " + logError("create user", e) + ". (The user-invite function may not be deployed yet.)"); }
    setBusy(false);
  }
...
function ClientPortal() {
  ...
  const load = useCallback(async () => { setLoading(true); setErr(""); try { setD(await invokeFunction("client-portal", { action: "overview" })); } catch (e) { setErr(logError("client portal", e)); } setLoading(false); }, []);
  useEffect(() => { load(); }, [load]);
  async function decide(appId: string, decision: string) { const fb = window.prompt("Add a note for the recruiter (optional):") || ""; try { await invokeFunction("client-portal", { action: "feedback", application_id: appId, decision, feedback: fb }); } catch (e) { reportError("client portal feedback", e, "Sending your decision"); } load(); }
  ...
  return <div className="min-h-screen bg-gray-50">
    ...
    <div className="p-6 max-w-4xl mx-auto">
      {loading ? <Loading /> : err ? <ErrorBox msg={`Couldn't load your portal: ${err}`} onRetry={load} /> : <div>
...
function Softphone() {
  ...
  // Driven by iframe events with no UI to report into, so failures are logged rather than shown.
  async function logZoom(p: any) {
    try {
      ...
      if (number) { const digits = String(number).replace(/[^0-9]/g, "").slice(-9); if (digits) { const rows = await mustRows<Row>("zoom candidate lookup", supabase.from("candidates").select("id").ilike("phone", `%${digits}%`).limit(1)); candidate_id = rows[0]?.id || null; } }
      await must("log zoom activity", supabase.from("activities").insert({ type: kind, description: `Zoom Phone ${kind}${number ? " · " + number : ""}${d.duration ? " · " + d.duration + "s" : ""}`, candidate_id, metadata: { zoom: true, event: p.type, raw: d } }));
    } catch (e) { logError("log zoom activity", e); }
  }
  useEffect(() => {
...
function ChatWidget() {
  ...
  async function send() {
    ...
    try { const data = await invokeFunction<Row>("chat", { messages: next }); setMsgs(m => [...m, { role: "assistant", content: data?.reply || "" }]); }
    catch (e) { setMsgs(m => [...m, { role: "assistant", content: "Sorry — " + logError("chat", e) }]); }
    setBusy(false);
  }
...
export default function Home() {
  const { profile, loading, error, reload, signOut } = useAuth();
  ...
  useEffect(() => { if (gq.trim().length < 2) { setGres([]); return; } const t = setTimeout(() => { mustRows<Row>("global search", supabase.from("candidates").select("id,first_name,last_name,current_title").or(`first_name.ilike.%${gq}%,last_name.ilike.%${gq}%,email.ilike.%${gq}%,current_title.ilike.%${gq}%`).limit(7)).then(setGres).catch(e => { setGres([]); logError("global search", e); }); }, 200); return () => clearTimeout(t); }, [gq]);
  if (loading) return <div className="min-h-screen flex items-center justify-center text-gray-400">Loading...</div>;
  // A session or profile failure must not fall through to the login screen, which
  // would look like a sign-out instead of the error it is.
  if (error && !profile) return <div className="min-h-screen flex items-center justify-center p-6"><div className="w-full max-w-md"><ErrorBox msg={`We couldn't load your account: ${error}`} onRetry={reload} /><button onClick={signOut} className="mt-3 text-xs text-gray-400 hover:text-gray-600">Sign out</button></div></div>;
  ...
  return <div className="min-h-screen bg-gray-50">
    <header className="bg-slate-900 text-white sticky top-0 z-30">
      <div className="flex items-center h-12 px-4 gap-1">
        ...
        <div className="relative shrink-0 hidden lg:block"><input value={gq} onChange={e => setGq(e.target.value)} placeholder="Search candidates…" className="w-44 focus:w-60 transition-all bg-white/10 placeholder-slate-400 text-white text-[13px] rounded px-3 py-1.5 outline-none" />{gres.length > 0 && <div className="absolute right-0 mt-1 w-72 bg-white text-gray-800 rounded-lg shadow-lg border divide-y z-40">{gres.map(r => <div key={r.id} onClick={() => nav("det", { id: r.id })} className="px-3 py-2 text-sm cursor-pointer hover:bg-gray-50"><div className="font-medium">{r.first_name} {r.last_name}</div><div className="text-[10px] text-gray-400">{r.current_title || ""}</div></div>)}</div>}</div>
