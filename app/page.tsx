...
function Cands({ nav, editable }: { nav: (p: string, d?: any) => void; editable: boolean }) {
  const [cs, setCs] = useState<C[]>([]); const [q, setQ] = useState(""); const [f, setF] = useState("all"); const [pg, setPg] = useState(0); const [tot, setTot] = useState(0); const [add, setAdd] = useState(false);
  ...
  return <div>
    ...
    <div className="bg-white rounded-xl border overflow-hidden"><table className="w-full text-sm"><thead><tr className="border-b text-left">{["Name", "Title", "Status", "Source", "Date"].map(h => <th key={h} className="px-3 py-2 text-[10px] font-semibold text-gray-400 uppercase">{h}</th>)}</tr></thead><tbody className="divide-y divide-gray-50">{cs.map(c => <tr key={c.id} onClick={() => nav("det", { id: c.id })} className="cursor-pointer hover:bg-gray-50"><td className="px-3 py-2"><div className="flex items-center gap-2"><Av n={`${c.first_name} ${c.last_name}`} sz="w-6 h-6 text-[9px]" /><div><div className="font-medium">{c.first_name} {c.last_name}</div><div className="text-[10px] text-gray-400">{c.email || ""}</div></div></div></td><td className="px-3 py-2 text-gray-500">{c.current_title || ""}</td><td className="px-3 py-2"><B s={c.status} /></td><td className="px-3 py-2 text-gray-400 capitalize text-xs">{c.source || ""}</td><td className="px-3 py-2 text-[10px] text-gray-400">{new Date(c.created_at).toLocaleDateString()}</td></tr>)}</tbody></table>
...
function Det({ nav, pr, editable }: { nav: (p: string, d?: any) => void; pr: any; editable: boolean }) {
  ...
  const [c, setC] = useState<C | null>(null); const [tab, setTab] = useState("overview"); const [apps, setApps] = useState<any[]>([]); const [notes, setNotes] = useState<any[]>([]); const [note, setNote] = useState(""); const [scoring, setScoring] = useState(false);
  ...
  return <div>
    ...
    <div className="bg-white rounded-xl border p-5 mb-4"><div className="flex gap-4"><Av n={`${c.first_name} ${c.last_name}`} sz="w-14 h-14 text-xl" /><div className="flex-1"><div className="flex items-center gap-3 mb-1 flex-wrap"><h1 className="text-lg font-semibold">{c.first_name} {c.last_name}</h1><B s={c.status} />{c.ai_recommendation && <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${c.ai_recommendation === "ADVANCE" ? "bg-green-100 text-green-700" : c.ai_recommendation === "REJECT" ? "bg-red-100 text-red-700" : "bg-yellow-100 text-yellow-700"}`}>{c.ai_recommendation}{c.overall_score ? ` ${c.overall_score}` : ""}</span>}</div><div className="text-sm text-gray-500">{c.current_title}{c.current_company ? ` at ${c.current_company}` : ""}</div><div className="flex gap-4 mt-2 text-xs text-gray-400 flex-wrap">{c.email && <span>{c.email}</span>}{c.phone && <span>{c.phone}</span>}{(c.city || c.state) && <span>{[c.city, c.state].filter(Boolean).join(", ")}</span>}{c.experience_years ? <span>{c.experience_years}yr exp</span> : null}</div></div>{editable && <select value={c.status} onChange={e => setStatus(e.target.value)} className="h-9 px-2 border rounded-lg text-xs self-start">{CAND_STATUSES.map(s => <option key={s} value={s}>{s.replace(/_/g, " ")}</option>)}</select>}</div></div>
...
function JobDetail({ nav, pr }: { nav: (p: string, d?: any) => void; pr: any }) {
  const [j, setJ] = useState<any>(null); const [apps, setApps] = useState<any[]>([]);
  useEffect(() => { (async () => { const { data } = await supabase.from("jobs").select("*,clients(company_name)").eq("id", pr.id).single(); setJ(data); const { data: ap } = await supabase.from("applications").select("id,status,created_at,candidate_id,candidates(first_name,last_name,current_title,status)").eq("job_id", pr.id); setApps(ap || []); })(); }, [pr.id]);
  ...
  return <div><button onClick={() => nav("jobs")} className="text-sm text-gray-400 mb-4 block">&larr; Back</button>
    <div className="bg-white rounded-xl border p-5 mb-4"><div className="flex justify-between items-start"><div><h1 className="text-lg font-semibold">{j.title}</h1><div className="text-sm text-gray-500 mt-1 capitalize">{j.clients?.company_name || ""}{j.location ? ` · ${j.location}` : ""}</div></div><B s={j.status} /></div>{j.description && <p className="text-sm text-gray-600 mt-4 whitespace-pre-wrap">{j.description}</p>}</div>
